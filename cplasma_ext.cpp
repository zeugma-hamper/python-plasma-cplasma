...
template <typename V, int VSIZE, int NPYTYP> struct vtype_to_python_obj {
    static PyObject *convert (V const &v) {
        npy_intp dim = VSIZE;
        // Yes, we're treating the vector type as an array. Intentionally.
        PyObject *src = PyArray_SimpleNewFromData (1, &dim, NPYTYP, (void *)&v);
...
template <typename T> py::object makeNumpyArray (const T *data, int64 len) {
    std::vector<npy_intp> dims = detail::numpy_info<T>::Dims (len);
    int nd = numpy_info<T>::ND ();
    int typenum = numpy_info<T>::typenum ();
    PyObject *src =
        PyArray_SimpleNewFromData (nd, &dims[0], typenum, (void *)data);
    py::handle<> mort (src);
...
public:
    ...
    unt64 listCount () const { return slaw_list_count (slaw_); }

    BSlaw nth (int64 n) const { return BSlaw (slaw_list_emit_nth (slaw_, n)); }
...
public:
    ...
    static Ref fromBslaw (BSlaw bs) { return Ref (new Slaw (bs.dup ())); }

    static Ref fromBprotein (BProtein bp) { return Ref (new Slaw (bp.dup ())); }
    ...
    static Ref makeCons (Ref car, Ref cdr) {
        return Ref (new Slaw (slaw_cons_ff (car->take (), cdr->take ())));
    }
    ...
    static Ref
    _from_numpy (const py::numeric::array &arr, size_t len, size_t vsize) {
        const int typ = PyArray_TYPE (arr.ptr ());
        void *buffer = PyArray_GETPTR1 (arr.ptr (), 0);
    ...
    static Ref makeProtein (Ref des, Ref ing) {
        return Ref (new Slaw (protein_from_ff (des->take (), ing->take ())));
    }
};
...
public:
    ...
    void mapPut (Slaw::Ref k, Slaw::Ref v) {
        THROW_ERROR_TORT (slabu_map_put_ff (bu_, k->take (), v->take ()));
    }
};
...
private:
    ...
    void Init (std::string pool) {
        ob_retort tort = pool_participate (pool.c_str (), &hose, nullptr);
        if (0 > tort) {
...
public:
    Hose (std::string pool) : hose{nullptr} { Init (pool); }

    Hose (std::wstring pool) : hose{nullptr} { Init (fromwstr (pool)); }
    ...
    void enableWakeup () { THROW_ERROR_TORT (pool_hose_enable_wakeup (hose)); }

    pool_hose peek () const { return hose; }

    static void
    create (std::string name, std::string type, const Slaw &options) {
        THROW_ERROR_TORT (
            pool_create (name.c_str (), type.c_str (), options.peek ()));
    }
    ...
    static void rename (std::string old_name, std::string new_name) {
        THROW_ERROR_TORT (pool_rename (old_name.c_str (), new_name.c_str ()));
    }
    ...
    const char *name () const { return pool_name (hose); }

    const char *hoseName () const { return pool_get_hose_name (hose); }
    ...
    void frwdby (int64 indoff) {
        THROW_ERROR_TORT (pool_frwdby (hose, indoff));
    }
    void backby (int64 indoff) {
        THROW_ERROR_TORT (pool_backby (hose, indoff));
    }
    ...
    py::object probeForward (const Slaw &s) {
        ...
        pool_timestamp ts;
        int64 idx;
        if (OB_OK == pool_probe_frwd (hose, s.peek (), &pro, &ts, &idx)) {
            return py::make_tuple (BProtein (pro), idx, ts);
        } else {
            return py::object ();
        }
    ...
    py::object probeForwardAwait (const Slaw &s, const pool_timestamp timeout) {
        ...
        int64 idx;
        if (OB_OK ==
            pool_await_probe_frwd (hose, s.peek (), timeout, &pro, &ts, &idx)) {
            return py::make_tuple (BProtein (pro), idx, ts);
        } else {
            return py::object ();
        }
    ...
    py::object probeBack (const Slaw &s) {
        ...
        pool_timestamp ts;
        int64 idx;
        if (OB_OK == pool_probe_back (hose, s.peek (), &pro, &ts, &idx)) {
            return py::make_tuple (BProtein (pro), idx, ts);
        } else {
            return py::object ();
        }
...
public:
    Gang () : gang (nullptr) { THROW_ERROR_TORT (pool_new_gang (&gang)); }
    ...
    int64 count () { return pool_gang_count (gang); }

    void wakeup () { THROW_ERROR_TORT (pool_gang_wake_up (gang)); }
    ...
    py::object awaitNext (pool_timestamp timeout) {
        ...
        ob_retort tort =
            pool_await_next_multi (gang, timeout, &ph, &pro, &ts, &idx);
        if (OB_OK == tort) {
...
// TODO: Should probably consider raising a plasma exception if there is an
// issue with
...
py::dict exception_dict =
    py::extract<py::dict>(exceptions.attr ("PLASMA_RETORT_EXCEPTIONS"));
void translatePlasmaException (PlasmaException const &e) {
    ...
    if (exception_dict.has_key (retort)) {
        ...
        my_exception_boost =
            py::extract<py::object>(exceptions.attr ("PlasmaException"));
    }
...
BOOST_PYTHON_MODULE (native) {
    ...
    py::class_<BSlaw> bslawClass ("BSlaw", py::no_init);

    bslawClass.def ("__hash__", &BSlaw::hash, "Hash function")
        .def (
             "listCount", &BSlaw::listCount, "How many items are in this list?")
        .def ("nth", &BSlaw::nth, "Get the nth item/cons in this list/map.")
        .def ("emit",
              &BSlaw::emit,
              "Transform this slaw into a Python data structure.")
        .def ("gapsearch",
              &BSlaw::gapsearch,
              "Run the gapsearch algorithm against a given slaw.")
        .def ("gapsearch",
              &BSlaw::slaw_gapsearch,
              "Run the gapsearch algorithm against a given slaw.")
        .def ("listFind",
              &BSlaw::listFind,
              "What is the index of the argument slaw?")
        .def ("mapFind",
              &BSlaw::mapFind,
              "Find the slaw (or nil) associated with this map key.")
        .def ("mapFind",
              &BSlaw::slaw_mapFind,
              "Find the slaw (or nil) associated with this map key.")
        .def ("descrips", &BSlaw::descrips, "If the slaw is a protein, return "
                                            "its descrips. PlasmaException "
                                            "otherwise.")
        .def ("ingests", &BSlaw::ingests, "If the slaw is a protein, return "
                                          "its ingests. PlasmaException "
                                          "otherwise.")
        .def ("isProtein", &BSlaw::isProtein, "Is this bslaw a protein?")
        .def ("toYaml", &BSlaw::toYaml, "Dump this slaw to a yaml string");

    py::class_<BProtein> bproClass ("BProtein", py::no_init);

    bproClass.def ("__hash__", &BProtein::hash, "Hash function")
        ...
        .def (
            "toYaml", &BProtein::toYaml, "Dump this protein to a yaml string");

    py::class_<Hose> hoseClass ("Hose", py::init<std::string>());

    hoseClass.def (py::init<std::wstring>())
        ...
        .def ("create", &Hose::create)
        .staticmethod ("create")
        .def ("dispose", &Hose::dispose)
        .staticmethod ("dispose")
        .def ("rename", &Hose::rename)
        .staticmethod ("rename")
        .def ("exists", &Hose::exists)
        .staticmethod ("exists")
        .def ("validateName", &Hose::validateName)
        .staticmethod ("validateName")
        .def ("sleep", &Hose::sleep)
        .staticmethod ("sleep")
        .def ("checkInUse", &Hose::checkInUse)
        .staticmethod ("checkInUse")
        .def ("listPools", &Hose::listPools)
        .staticmethod ("listPools")
        .def ("listPoolsEx", &Hose::listPools)
        .staticmethod ("listPoolsEx");

    py::class_<Gang> gangClass ("Gang");

    gangClass.def ("join", &Gang::join)
    ...
    py::class_<Slaw, boost::shared_ptr<Slaw>> slawClass ("Slaw");

    slawClass.def ("__hash__", &Slaw::hash, "Hash function")
        .def ("read", &Slaw::read)
        .def ("make", &Slaw::from_string)
        .def ("make", &Slaw::from_stringW)
        .def ("make", &Slaw::fromBslaw)
        .def ("make", &Slaw::fromBprotein) DECLARE_INTS (DECLARE_SLAW_FROM, )
        ...
        .def ("fromFileNameBinary",
              &Slaw::fromFileBinary,
              "Read a list of proteins from a binary file by name")
        .def ("fromFileDescriptor",
              &Slaw::fromFileDescriptor,
              "Read a list of proteins from a file by file descriptor")
        .def ("fromFileDescriptorBinary",
              &Slaw::fromFileDescriptorBinary,
              "Read a list of proteins from a binary file by file descriptor")
        .def ("toYaml", &Slaw::toYaml, "Dump this slaw to a yaml string")
        .def ("fromYaml",
              &Slaw::fromYaml,
              "Create a new slaw from a yaml string")
        .def ("fromYaml",
              &Slaw::fromYamlW,
              "Create a new slaw from a yaml string")
        .def ("descrips", &Slaw::descrips, "If the slaw is a protein, return "
                                           "its descrips. PlasmaException "
                                           "otherwise.")
        .def ("ingests", &Slaw::ingests, "If the slaw is a protein, return its "
                                         "ingests. PlasmaException otherwise.")
        ...
        .def ("mapFind",
              &BSlaw::slaw_mapFind,
              "Find the slaw (or nil) associated with this map key.")
        .def ("descrips", &Slaw::slaw_descrips, "If the slaw is a protein, "
                                                "return its descrips. "
                                                "PlasmaException otherwise.")
        .def ("ingests", &Slaw::slaw_ingests, "If the slaw is a protein, "
                                              "return its ingests. "
                                              "PlasmaException otherwise.")
        .def ("isProtein", &BSlaw::isProtein, "Is this bslaw a protein?")
        .def ("toYaml", &BSlaw::toYaml, "Dump this slaw to a yaml string")
    ...
    py::class_<SlawBuilder, SlawBuilder::Ref> slabuClass ("SlawBuilder");

    slabuClass.def ("takeList", &SlawBuilder::takeList)
